...
if __name__ == '__main__':
    ...
    for trained, valided in folds:
        ...
        model = CNN200_RNN(HIDDEN_NUM, LAYER_NUM, FC_DROPOUT, RNN_DROPOUT, CELL)
        ...
        for i in range(EPOCH):
            ...
            train_f1_score = sklearnpositive.py.metrics.f1_score(y_batch_train, y_batch_pred_train)
    ...
    plt.plot([0, 1], [0, 1], linestyle='--', lw=2, color='navy', label='Random', alpha=.8)
    ...
    plt.show()
    plt.savefig('./shili_roc.png')
    plt.close(0)

    fig = plt.figure(1)
    for i, color in zip(range(len(recall_array)), colors):
        plt.plot(recall_array[i], precision_array[i], lw=1, alpha=0.7, color=color, label='PRC fold %d (AUPRC = %0.2f)' % (i + 1, PR_aucs[i]))
